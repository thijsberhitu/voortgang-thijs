...
const salt=randomBytes(16).toString('hex'), password='test-only-password';
const vars={...process.env,DEMO_MODE:'false',NODE_ENV:'test',PORT:'8098',DATA_DIR:mkdtempSync(join(tmpdir(),'vt-api-test-')),TEAM_PASSWORD_HASH:salt+':'+scryptSync(password,salt,64).toString('hex'),SESSION_SECRET:randomBytes(40).toString('hex'),SYNC_TOKEN:randomBytes(40).toString('hex')};
test('Authenticatie, volledige snapshots, retries en opslag na herstart',async()=>{
  ...
  try{
    ...
    assert.equal(view.snapshots.length,2);assert.equal(view.clients[0].change.kind,'up');assert.equal(view.clients[0].history[0].priority,4);
    server.kill('SIGTERM');await once(server,'exit');await start();
    const persisted=await (await request('/api/dashboard',null,{Cookie:cookie})).json();assert.equal(persisted.snapshots.length,2);
    assert.equal(persisted.clients[0].priority,1);
